...
public class NegativeAnswer : VirtualWorldBoxAction
{
        ...
        if (face_trgr.Success)
        {
            ...
            else if (gesture1Completed && gesture1_left)
            {
                ...
                if (correct_move_right2 && !resetNeeded)
                {
                    ...
                    // We show the no answer and we hide the rest
                    ...
                    answerYes.GetComponent<MeshRenderer>().enabled = false;

                    GameObject answerNo = GameObject.FindGameObjectWithTag("AnswerNo");
                    answerNo.GetComponent<MeshRenderer>().enabled = true;
                }
