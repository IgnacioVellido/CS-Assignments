...
const int num_items = 40 ,   		// Número de items
	  tam_vec   = num_items / 2 ,   // Tamaño del buffer
	  TOTAL_HEBRAS = 2 ;		// Número de hebras
...
Semaphore sem_pro[TOTAL_HEBRAS] = {tam_vec,tam_vec}, 
	  sem_con[TOTAL_HEBRAS] = {0,0} ;	
...
{
   int puntero_pro[TOTAL_HEBRAS] = {0,0} ;		// Inicializando el punto de comienzo de la inserción de valores
   ...
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      ...
      if ((i % 2) == 0)
	buffer_a_insertar = 0;				// Si el nºelemento es par se añade al primer buffer
      else
	buffer_a_insertar = 1;
