...
% Viendo los resultados obtenidos, y tras algunas pruebas (más detalles en el apartado \textit{Conclusiones}), no se considera necesario modificarlo por un esquema proporcional, pues aunque al principio el descenso es alto resulta suficiente para evitar los mínimos locales y hacer coverger en mejores soluciones. Unas pequeñas pruebas con el esquema proporcional corroboran que no es necesario aplicarlo.

\begin{algorithm}[H]
    \SetAlgoLined
    \KwIn{initialTemperature, minTemperature, maxAnnealing}
...
\begin{algorithm}[H]
    \SetAlgoLined
    \KwIn{temperature, $\beta$}
        % newTemperature $=$ temperature $/$ ($1 + \beta$ * temperature) \;
...
\begin{algorithm}[H]
    \SetAlgoLined
    \KwIn{actualCost, newCost, temperature}
...
\begin{algorithm}[H]
    \SetAlgoLined
    \SetKwRepeat{Do}{do}{while}
    \KwIn{solution, número de clusters}    
        \Do{newSolution no es válida} {
...
\begin{algorithm}[H]
    ...
    \KwIn{Conjunto de datos, lista de restricciones}
        ...
        \BlankLine \BlankLine
        \RepTimes{maxAnnealing} {
            successes $=$ $0$ \;
            \RepTimes{maxNeighbors} {
                newSolution $=$ generateNeighbor(actualSolution) \;
                newCost $=$ cost(newSolution) \;
                \BlankLine \BlankLine
                \If{metrop(actualCost, newCost, temperature)} {
                    successes++ \;
                    actualCost $=$ newCost \;
...
\begin{algorithm}[H]
    \SetAlgoLined
    \KwIn{solution}
        \While{exista clúster vacío en solution} {
...
\begin{algorithm}[H]
    ...
    \KwIn{Conjunto de datos, lista de restricciones}
        ...
        \RepTimes{executions} {
            ...
            repair(bestSolution) \;
            \BlankLine \BlankLine
            \tcc{Preparar siguiente iteración  - - - - - - - - - - - - - -}
            solution $=$ bestSolution \;
            cost $=$ bestCost $=$ Coste de bestSolution \;
            \BlankLine \BlankLine
            \tcc{Llamada al algoritmo de BL - - - - - - - - - - - - - - - -}
            centroids $=$ Conjunto con los centroides de cada clúster \;
            evaluations $=$ 0 \;
            \BlankLine \BlankLine
            \While{solution cambie \textbf{and} $evaluations < 10\,000$} {
            ...
            \BlankLine \BlankLine
            \tcc{Comprobar si la solución mejora - - - - - - - - - - - - -}
            \If{bestCost $>$ cost} {
                bestCost $=$ cost \;
                bestSolution $=$ solution \;
            }
...
El algoritmo híbrido simplemente sustituye la inclusión de la BL por una llamada al algoritmo ES. El operador de mutación y de reparación se mantienen de la misma forma, y la inicialización queda de la siguiente manera:
...
\begin{algorithm}[H]
    ...
    \KwIn{Conjunto de datos, lista de restricciones}
        ...
        \RepTimes{executions} {
            ...
            \BlankLine \BlankLine
            \tcc{Reiniciamos temperatura y llamamos a ES - - - - - - - - -}
            temperature $=$ initialTemperature(actualCost) \;
            ...
            \BlankLine \BlankLine
            \tcc{Comprobar si hay mejora (bestSolution contiene la mejor solución de la llamada a ES, trueBestSolution la mejor en general)}
            \If{trueBestCost $>$ bestCost} {
                trueBestCost $=$ bestCost \;
                trueBestSolution $=$ bestSolution \;
            }
        }
    \Return{trueBestSolution}
