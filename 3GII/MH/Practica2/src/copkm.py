...
def algorithm(data, rest_matrix, rest_list, num_clust, rng, agg=True, uniform=True, amType="10"):
    ...
    while solution_changed:
        ...
        # Con esto desechamos la solución anterior y mejoramos únicamente en 
        # base a los centroides
        solution = np.full(len(data), -1)
        ...
        if not np.array_equal(solution, old_solution):
