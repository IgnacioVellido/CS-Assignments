...
_triangulos3D::_triangulos3D() { 
  // Materiales
  float bri = 10;
  _vertex4f dif = {0,0.4,0,1};
  _vertex4f esp = {1,1,1,1};

  ambiente_difusa.push_back(dif);
  especular.push_back(esp);
  brillo.push_back(bri);
...
//*************************************************************************
// Rotar por los materiales
//*************************************************************************


void _triangulos3D::cambiar_material() {
  material = (material+1) % (brillo.size());
}
...
void _triangulos3D::draw_iluminacion_plana() {  
  ...
  glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (GLfloat *)  &(ambiente_difusa[material]));
  glMaterialfv(GL_FRONT, GL_SPECULAR, (GLfloat *) &(especular[material]));
  glMaterialf(GL_FRONT, GL_SHININESS, brillo[material]);
...
void _triangulos3D::calcular_normales_vertices() {  
  ...
  float x=0, y=0, z=0, m=0;
	...
	for(int i=0; i<v_size; i++) {
    x = normales_vertices[i].x;
    y = normales_vertices[i].y;
    z = normales_vertices[i].z;
    m = std::sqrt(x*x + y*y + z*z);

    normales_vertices[i] = _vertex3f(x/m, y/m, z/m);
  }
...
void _atat::cambiar_material() {
  cabeza.cambiar_material();
  pierna.cambiar_material();
  chasis2.cambiar_material();
}
...
void _cabeza::cambiar_material() {
  cabeza.cambiar_material();
  lateral_cabeza.cambiar_material();
  torreta.cambiar_material();
}
...
void _chasis2::cambiar_material() {
  cuerpo.cambiar_material();
}
...
void _pierna::cambiar_material() {
  parte_inferior.cambiar_material();
  parte_superior.cambiar_material();
  pie.cambiar_material();
}
